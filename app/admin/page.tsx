...
export default function AdminDashboardPage() {
  ...
  // Años disponibles según los pedidos existentes
  ...
  // Pedidos filtrados por mes/año si el filtro está activo
  ...
  // Derived metrics — solo pedidos efectivamente entregados cuentan como ingreso
  ...
  // Top products — solo pedidos entregados cuentan como vendidos
    ...
    .forEach((o) =>
      o.items.forEach((item) => {
        if (!productSales[item.productId]) {
          productSales[item.productId] = { name: item.name, qty: 0 };
        }
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {filterActive && (
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 font-label text-[10px] tracking-widest uppercase border border-primary/40 text-primary hover:bg-primary/10 transition-colors"
            >
          ...
          <select
            value={selectedMonth}
            onChange={(e) => { setSelectedMonth(Number(e.target.value)); setFilterActive(true); }}
            className="bg-surface-container border border-outline-variant/40 px-3 py-2 font-label text-xs tracking-widest text-on-surface focus:outline-none focus:border-primary"
          >
            {MONTHS.map((m, i) => (
          ...
          <select
            value={selectedYear}
            onChange={(e) => { setSelectedYear(Number(e.target.value)); setFilterActive(true); }}
            className="bg-surface-container border border-outline-variant/40 px-3 py-2 font-label text-xs tracking-widest text-on-surface focus:outline-none focus:border-primary"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 px-4 py-2 font-label text-[10px] tracking-widest uppercase bg-primary text-on-primary hover:bg-primary-container transition-colors"
          >
      ...
      {filterActive && (
        <div className="flex items-center gap-2 px-4 py-2 bg-primary/10 border border-primary/30 w-fit">
          ...
          <span className="font-label text-xs tracking-widest uppercase text-primary">
            Mostrando: {MONTHS[selectedMonth]} {selectedYear}
          </span>
        </div>
      )}
            ...
            <div className="space-y-4">
              {topProducts.map(([, data], i) => (
                <div key={i}>
                  <div className="flex justify-between items-center mb-1">
                    <span
                      className="font-label text-xs text-on-surface-variant truncate max-w-[70%]"
                      title={data.name}
                    >
                      {data.name}
                    </span>
      ...
      <div className="bg-surface-container-low border border-outline-variant/20">
        <div className="flex items-center justify-between px-6 py-4 border-b border-outline-variant/20">
          ...
          <Link
            href="/admin/orders"
            className="font-label text-xs tracking-widest uppercase text-primary hover:text-on-primary-container transition-colors"
          >
            Ver todos →
          </Link>
        </div>
            ...
            {recentOrders.map((order) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div>
                    <div className="font-label text-xs font-bold text-on-surface tracking-widest">
                      {order.id}
                    </div>
                ...
                <div className="flex items-center gap-4">
                  <span
                    className={`font-label text-[10px] tracking-widest uppercase px-2 py-1 border ${STATUS_COLORS[order.status]}`}
                  >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <StatCard
          ...
          label="Stock bajo"
          value={
            products.filter((p) => p.status === "low-stock").length.toString()
          }
          accent="text-yellow-400"
        />
        <StatCard
  ...
  return (
    <div className="bg-surface-container-low border border-outline-variant/20 p-5 space-y-3">
      <div className="flex items-center gap-2">
        <span className={`material-symbols-outlined text-[20px] ${accent}`}>
          {icon}
        </span>
      ...
      <div
        className={`font-headline font-black tracking-tight ${accent} ${small ? "text-xl" : "text-3xl"}`}
      >
        {value}
      </div>
    </div>
