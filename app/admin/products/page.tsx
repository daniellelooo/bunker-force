          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-outline-variant/20">
                {["Producto", "SKU", "Categoría", "Precio", "Estado", "Destacado", "Acciones"].map(
                  (h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 font-label text-[10px] tracking-[0.2em] uppercase text-outline"
                    >
                      {h}
                    </th>
                  )
            ...
            <tbody className="divide-y divide-outline-variant/10">
              {products.map((product) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      {product.images[0] && (
                        ...
                        <img
                          ...
                          alt={product.name}
                          className="w-10 h-10 object-cover bg-surface-container-high shrink-0"
                        />
                      )}
                      <div>
                        ...
                        {product.badge && (
                          <div className="font-label text-[9px] tracking-widest text-primary uppercase mt-0.5">
                            {product.badge}
                          </div>
                        )}
                      </div>
                  ...
                  <td className="px-4 py-4">
                    <span
                      className={`font-label text-[10px] tracking-widest uppercase px-2 py-1 border ${STATUS_COLORS[product.status]}`}
                    >
                  ...
                  <td className="px-4 py-4">
                    <button
                      ...
                      title={product.featured ? "Quitar de destacados" : "Destacar en página principal"}
                      className="transition-transform hover:scale-110 active:scale-95 disabled:opacity-40"
                    >
                      <span
                        className={`material-symbols-outlined text-[22px] ${product.featured ? "text-primary" : "text-outline hover:text-primary"}`}
                      >
            ...
            >
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="font-label text-xs text-outline tracking-widest mt-0.5">
                    {product.sku}
                  </div>
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    ...
                    <span
                      className={`font-label text-[10px] tracking-widest uppercase px-2 py-0.5 border ${STATUS_COLORS[product.status]}`}
                    >
                      {STATUS_LABELS[product.status]}
                    </span>
                  </div>
                ...
                <button
                  onClick={() => handleToggleFeatured(product)}
                  disabled={togglingId === product.id}
                  className="transition-transform active:scale-95 disabled:opacity-40 shrink-0"
                >
                  <span
                    className={`material-symbols-outlined text-[22px] ${product.featured ? "text-primary" : "text-outline"}`}
                  >
                    {product.featured ? "star" : "star_outline"}
                  </span>
                </button>
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-outline-variant/20">
                <Link
                  href={`/admin/products/${product.id}/edit`}
                  className="flex-1 flex items-center justify-center gap-1 py-2 font-label text-[10px] tracking-widest uppercase border border-outline-variant/40 text-outline hover:text-on-surface hover:border-outline transition-colors"
                >
                ...
                <button
                  onClick={() => handleDelete(product.id, product.name)}
                  disabled={deletingId === product.id}
                  className="flex-1 flex items-center justify-center gap-1 py-2 font-label text-[10px] tracking-widest uppercase border border-error/30 text-error/60 hover:text-error hover:border-error transition-colors disabled:opacity-40"
                >
